...
void _again(int len, ...) {
    SHOWOP("AGAIN");
    DATA(BRAN);
    DATA(RPOP());                   // store return address
    DATACPY(len);
}
void _until(int len, ...) {
    SHOWOP("UNTIL");
    DATA(QBRAN);                   // conditional branch
    DATA(RPOP());                   // loop begin address
    DATACPY(len);
}
void _while(int len, ...) {
    SHOWOP("WHILE");
...
int assemble(U8 *rom) {
	...
	XA vHLD  = _CODE("HLD",     opDOCON, opNEXT, VAR(ta,0), 0);
	XA vSPAN = _CODE("SPAN",    opDOCON, opNEXT, VAR(ta,1), 0);
	XA vIN   = _CODE(">IN",     opDOCON, opNEXT, VAR(ta,2), 0);
	XA vNTIB = _CODE("#TIB",    opDOCON, opNEXT, VAR(ta,3), 0);
	XA ua    = FORTH_UVAR_ADDR;
	XA vTTIB = _CODE("'TIB",    opDOCON, opNEXT, VAR(ua,0), 0);
	XA vBASE = _CODE("BASE",    opDOCON, opNEXT, VAR(ua,1), 0);
	XA vCNTX = _CODE("CONTEXT", opDOCON, opNEXT, VAR(ua,2), 0);
	XA vCP   = _CODE("CP",      opDOCON, opNEXT, VAR(ua,3), 0);
	XA vLAST = _CODE("LAST",    opDOCON, opNEXT, VAR(ua,4), 0);
	XA vTEVL = _CODE("'EVAL",   opDOCON, opNEXT, VAR(ua,5), 0);
	XA vTABRT= _CODE("'ABORT",  opDOCON, opNEXT, VAR(ua,6), 0);
	XA vTEMP = _CODE("tmp",     opDOCON, opNEXT, VAR(ua,7), 0);
	//
	// Kernel dictionary (primitive proxies)
	//
	   NOP   = _CODE("NOP",     opNOP,   opNEXT);
	XA BYE   = _CODE("BYE",     opBYE,   opNEXT);
	XA QRX   = _CODE("?RX",     opQRX,   opNEXT);
	XA TXSTO = _CODE("TX!",     opTXSTO, opNEXT);
	XA DOCON = _CODE("DOCON",   opDOCON, opNEXT);
	XA DOLIT = _CODE("DOLIT",   opDOLIT, opNEXT);
	XA ENTER = _CODE("ENTER",   opENTER, opNEXT);    // aka DOLIST by Dr. Ting
	XA EXIT  = _CODE("EXIT",    opEXIT,  opNEXT);
	XA EXECU = _CODE("EXECUTE", opEXECU, opNEXT);
	   DONXT = _CODE("DONEXT",  opDONEXT,opNEXT);
	   QBRAN = _CODE("QBRANCH", opQBRAN, opNEXT);
	   BRAN  = _CODE("BRANCH",  opBRAN,  opNEXT);
	XA STORE = _CODE("!",       opSTORE, opNEXT);
	XA AT    = _CODE("@",       opAT,    opNEXT);
	XA CSTOR = _CODE("C!",      opCSTOR, opNEXT);
	XA CAT   = _CODE("C@",      opCAT,   opNEXT);
	XA RFROM = _CODE("R>",      opRFROM, opNEXT);
	XA RAT   = _CODE("R@",      opRAT,   opNEXT);
	   TOR   = _CODE(">R",      opTOR,   opNEXT);
	XA DROP  = _CODE("DROP",    opDROP,  opNEXT);
	XA DUP   = _CODE("DUP",     opDUP,   opNEXT);
	XA SWAP  = _CODE("SWAP",    opSWAP,  opNEXT);
	XA OVER  = _CODE("OVER",    opOVER,  opNEXT);
	XA ZLESS = _CODE("0<",      opZLESS, opNEXT);
	XA AND   = _CODE("AND",     opAND,   opNEXT);
	XA OR    = _CODE("OR",      opOR,    opNEXT);
	XA XOR   = _CODE("XOR",     opXOR,   opNEXT);
	XA UPLUS = _CODE("UM+",     opUPLUS, opNEXT);
	XA NEXT  = _CODE("NEXT",    opNEXT,  opNEXT);
	XA QDUP  = _CODE("?DUP",    opQDUP,  opNEXT);
	XA ROT   = _CODE("ROT",     opROT,   opNEXT);
	XA DDROP = _CODE("2DROP",   opDDROP, opNEXT);
	XA DDUP  = _CODE("2DUP",    opDDUP,  opNEXT);
	XA PLUS  = _CODE("+",       opPLUS,  opNEXT);
	XA INVER = _CODE("NOT",     opINVER, opNEXT);
	XA NEGAT = _CODE("NEGATE",  opNEGAT, opNEXT);
	XA DNEGA = _CODE("DNEGATE", opDNEGA, opNEXT);
	XA SUB   = _CODE("-",       opSUB,   opNEXT);
	XA ABS   = _CODE("ABS",     opABS,   opNEXT);
	XA EQUAL = _CODE("=",       opEQUAL, opNEXT);
	XA ULESS = _CODE("U<",      opULESS, opNEXT);
	XA LESS  = _CODE("<",       opLESS,  opNEXT);
	XA UMMOD = _CODE("UM/MOD",  opUMMOD, opNEXT);
	XA MSMOD = _CODE("M/MOD",   opMSMOD, opNEXT);
	XA SLMOD = _CODE("/MOD",    opSLMOD, opNEXT);
	XA MOD   = _CODE("MOD",     opMOD,   opNEXT);
	XA SLASH = _CODE("/",       opSLASH, opNEXT);
	XA UMSTA = _CODE("UM*",     opUMSTA, opNEXT);
	XA STAR  = _CODE("*",       opSTAR,  opNEXT);
	XA MSTAR = _CODE("M*",      opMSTAR, opNEXT);
	XA SSMOD = _CODE("*/MOD",   opSSMOD, opNEXT);
	XA STASL = _CODE("*/",      opSTASL, opNEXT);
	XA PICK  = _CODE("PICK",    opPICK,  opNEXT);
	XA PSTOR = _CODE("+!",      opPSTOR, opNEXT);
	XA DSTOR = _CODE("2!",      opDSTOR, opNEXT);
	XA DAT   = _CODE("2@",      opDAT,   opNEXT);
	XA COUNT = _CODE("COUNT",   opCOUNT, opNEXT);
	XA MAX   = _CODE("MAX",     opMAX,   opNEXT);
	XA MIN   = _CODE("MIN",     opMIN,   opNEXT);

	XA BLANK = _CODE("BL",      opDOCON, opNEXT, 0x20,   0);
	XA CELL  = _CODE("CELL",    opDOCON, opNEXT, CELLSZ, 0);
	XA CELLP = _CODE("CELL+",   opDOCON, opPLUS, opNEXT, CELLSZ, 0);
	XA CELLM = _CODE("CELL-",   opDOCON, opSUB,  opNEXT, CELLSZ, 0);
	XA CELLS = _CODE("CELLS",   opDOCON, opSTAR, opNEXT, CELLSZ, 0);
	XA CELLD = _CODE("CELL/",   opDOCON, opSLASH,opNEXT, CELLSZ, 0);
	XA ONEP  = _CODE("1+",      opDOCON, opPLUS, opNEXT, 1,      0);
	XA ONEM  = _CODE("1-",      opDOCON, opSUB,  opNEXT, 1,      0);
	XA DOVAR = _CODE("DOVAR",   opDOVAR, opNEXT);
	//
	// tracing instrumentation (borrow 2 opcodes)
	//
	XA clock   = _CODE("clock",   opSPAT,  opNEXT);
	XA trc_on  = _CODE("trc_on",  opRPAT,  opNEXT);
	XA trc_off = _CODE("trc_off", opRPSTO, opNEXT);
	//
	...
	XA ALIGN = _COLON("ALIGNED", DOLIT, 3, PLUS, DOLIT, 0xfffffffc, AND, EXIT);
	...
	XA DOSTR = _COLON("do$",   RFROM, RAT, RFROM, COUNT, PLUS, ALIGN, TOR, SWAP, TOR, EXIT);
	...
	XA NAMET = _COLON("NAME>", COUNT, DOLIT, 0x1f, AND, PLUS, ALIGN, EXIT);
	...
	*/
	...
	XA ALLOT = _COLON("ALLOT",   ALIGN, vCP, PSTOR, EXIT);
	XA STRCQ = _COLON("$,\"",    DOLIT, 0x22, WORDD, COUNT, PLUS, ALIGN, vCP, STORE, EXIT);
