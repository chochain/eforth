...
const static Code primitives[] = {
	...
	CODE("negate",top = -top),
	CODE("max",   int n = top; pop; top = std::max(top, n)),
	CODE("min",   int n = top; pop; top = std::min(top, n)),
	CODE("2*",    top *= 2),
	CODE("2/",    top /= 2),
	CODE("1+",    top += 1),
	CODE("1-",    top += -1),
	...
	/// IO ops
	CODE("base@", push base),
	CODE("base!", base = top; pop; std::cout << std::setbase(base)),
	CODE("hex",   base = 16; std::cout << std::setbase(base)),
	CODE("decimal", base = 10; std::cout << std::setbase(base)),
	CODE("cr",    std::cout << std::endl),
	CODE(".",     std::cout << top << " "; pop),
	CODE(".r",    int n = top; pop; dot_r(n, top); pop),
	CODE("u.r",   int n = top; pop; dot_r(n, abs(top)); pop),
	CODE(".s",    ss_dump()),
	CODE("key",   push(next_idiom()[0])),
	CODE("emit",  char b = (char)top; pop; std::cout << b),
	CODE("space", std::cout << " "),
	CODE("spaces",int n = top; pop; for (int i = 0; i < n; i++) std::cout << " "),
	/// Literal ops
	CODE("dostr",
